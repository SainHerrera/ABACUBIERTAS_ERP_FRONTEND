...
const outDir = join(root, 'test-results', 'compras-audit')

console.log('▶ Ejecutando E2E del paquete de Compras (headless)...')
let raw = ''
try {
  raw = execFileSync('npx', ['playwright', 'test', 'e2e/compras', '--reporter=json'], {
    cwd: root,
    encoding: 'utf8',
    shell: true,
...
const lines = []
lines.push('===========================================================')
lines.push(' AUDITORÍA DEL PAQUETE DE COMPRAS (E2E)')
...
writeFileSync(join(outDir, 'compras-audit-report.txt'), reportTxt, 'utf8')
writeFileSync(join(outDir, 'results.json'), JSON.stringify(data, null, 2), 'utf8')

console.log(reportTxt)
console.log('')
console.log('Archivos de auditoría generados en: test-results/compras-audit/')
